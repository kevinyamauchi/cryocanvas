...
# Open the Zarr file and load the datasets
...
def threaded_on_data_change(
    ...
    def compute_features(mask_idx, use_skimage_features, use_tomotwin_features):
        ...
        if use_skimage_features:
            features.append(feature_data_skimage[mask_idx].reshape(-1, feature_data_skimage.shape[-1]))
        if use_tomotwin_features:
            features.append(feature_data_tomotwin[mask_idx].reshape(-1, feature_data_tomotwin.shape[-1]))
    ...
    if data_choice == "Current Displayed Region":
        ...
        training_features = compute_features(
            active_image, mask_idx, False, True
        )
